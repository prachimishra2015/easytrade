...
class CustomStockTooltip extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.active && this.props.payload[0]) {
      ...
      let priceFluxString = `${neg}$${Math.abs(priceFluxCalc).toFixed(2)} (${priceFluxPercentageCalc.toFixed(2)}%)`
      price.innerHTML = `$${parseFloat(this.props.payload[0].value).toFixed(2)}`;
